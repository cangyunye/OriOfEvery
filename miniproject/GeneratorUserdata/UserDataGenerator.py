...
logging.basicConfig(filename=os.path.join(sqlfdir, f"UDG_{dt.strftime(" % Y % m % d % H")}.log"), filemode='a',
					level=nameToLevel['INFO'], format='%(asctime)s %(level)-8s%(funcName)s:%(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
class UserDataGenerator():
	...
	def getinfo(self, servnumber):
		...
		:return:()
		"""
		logger.info(f"Get info of  {servnumber} from database.")
		oracleconn = eval(self.init_dict['oracleconn'])
		logger.info(f"Database infomation: {self.init_dict['oracleconn']}")
		timestenconn = eval(self.init_dict['timestenconn'])
		logger.info(f"Database infomation: {self.init_dict['timestenconn']}")
		...
		subsl = []
		subs_info = namedtuple('subs', [
							   'servnumber', 'subsid', 'acctid', 'custid', 'region', 'brand', 'active', 'nodeid'])

		# 用户基本信息
		subs_sql = f"select servnumber,subsid,acctid,custid,region,brand from hsc_subs_subscriber where servnumber='{servnumber}';"
		logger.info(f"subs_sql: {subs_sql}")
		tmp00 = ret(DBT, subs_sql)
		if 'None' not in tmp00:
			subsl += ret(DBT, subs_sql) + \
				[self.init_dict['active'], self.init_dict['nodeid']]
		else:
			os._exit(0)
		# 元组组装
	...
	def replace_dict(self, cfgfile='GlobalSettings.ini'):
		...
		logger.info(f"Init GlobalVariables from {cfgfile}")

		# 用户数据段
		logger.info(f"Load sqlvar.")
		for col in cfg['sqlvar']:
			if self.init_dict[col]:
				continue
			self.init_dict[col] = cfg['sqlvar'][col]
			
		# eval解析段
		logger.info(f"Load pyeval.")
	...
	def selfclean(self, cfg='clean_Common_0001.json'):
		# 清理表，通用清理方案
		logger.info(f"Appending clean sqlscripts.")
		jdata = self.readcfg(cfg=cfg)
		u = self.cfgparser(jdata)
		for tbdsql, DbDriver in u:
			logger.debug(f"DbDriver={DbDriver},tbdsql={tbdsql}")
	...
	def cbrand(self, cfg='base_BrandSzx_0001.json'):
		logger.info(f"Appending brand sqlscripts.")
		if self.init_dict['brand'].upper() == 'SZX':
			...
			logger.error(f"May be wrong with cfgfiles.")

	def cactive(self, cfg):
		logger.info(f"Appending active sqlscripts.")
	...
	def croute(self, cfg='route_BrandSzx_0001.json'):
		logger.info(f"Appending route sqlscripts.")
		# 调用sqlplus查询，或者直接给定状态
	...
	def busidata(self, cfg='base_prod206_0001.json'):
		# 选择业务数据生成
		logger.info(f"Appending busi sqlscripts.")
	...
	def generator(self, sqllist):
		...
		:return:
		"""
		logger.info(f"Begin Generating Files.")
	...
	def process(self, **kwargs):
		...
		if kwargs.get('mode','imp') == 'imp':
			logger.info("implement mode.")
			# 获取用户现存资料
			if kwargs.get('servnumber', None):
				self.getinfo(kwargs['servnumber'])
				self.init_dict['subsprodid'] = kwargs['servnumber']
			else:
			...
			if kwargs.get('active', None):
				self.cactive(kwargs['active'])
			else:
				self.cactive(self.init_dict['servnumber'])
			# 是否有路由信息
			if kwargs.get('nodeid', None):
				self.croute(kwargs['nodeid'])
			else:
				self.croute(self.init_dict['nodeid'])
			# 数据清理
			if kwargs.get('scfg', None):
				self.selfclean(kwargs['scfg'])
			else:
				self.selfclean(self.init_dict['scfg'])
			# 业务数据
			if kwargs.get('bcfg', None):
				self.busidata(kwargs['bcfg'])
			else:
			self.busidata(self.init_dict['bcfg'])
		elif kwargs.get('mode',None) == 'rec':
